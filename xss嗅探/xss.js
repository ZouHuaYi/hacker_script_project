...
function testXssCode() {
  const urlcode = createXssCode(`<>'"`)
  const endUrl = urlcode.map(url => {
    const u = `zfyi${url}zfyi`
    return `https://sou.autohome.com.cn/zonghe?q=${u}&mq=&pvareaid=3311224`
  })
  console.log(endUrl)
  endUrl.forEach(url => {
    request({
